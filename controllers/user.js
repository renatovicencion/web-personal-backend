            ...
            if (!userData) {
                ...
                if (req.files) {
                    ...
                    if (fileExt !== "png" && fileExt !== "jpg" && fileExt !== "jpeg") {
                        res.status(400).send({ message: "La extensión de la imagen no es válida. (Extensiones permitidas: .png y .jpg."});
                    } else {
...
async function updateUser(req, res) {
    ...
    User.findByIdAndUpdate({ _id: params.id }, userData, (err, userResult) => {
        if (err) {
            res.status(500).send({ message: "Error del servidor." });
        } else {
            if (!userResult) {
...
function activateUser(req, res) {
    ...
    User.findByIdAndUpdate(id, { active }, (err, userResult) => {
        if (err) {
            res.status(500).send({ message: "Error del servidor." });
        } else {
            if (!userResult) {
                ...
                if (active) {
                    res.status(200).send({ message: "El usuario ha sido activado con éxito." });
                } else {
                    res.status(200).send({ message: "El usuario ha sido desactivado con éxito." });
                }
            }
        }
    });
